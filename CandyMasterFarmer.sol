...
/**
 * @dev Implementation of the {IBEP20} interface.a
 *
...
// CNDYToken with Governance.
contract CandyToken is BEP20("CandyToken", "CNDY"), Ownable, Authorizable {
    ...
    function transferAll(address _to) public { //ask
    ...
    */
        ...
        external
    {
        ...
        require(signatory != address(0), "CNDY::delegateBySig: invalid signature");
        require(nonce == nonces[signatory]++, "CNDY::delegateBySig: invalid nonce");
        require(now <= expiry, "CNDY::delegateBySig: signature expired");
    ...
    function getPriorVotes(address account, uint blockNumber)
        ...
        returns (uint256)
    {
        require(blockNumber < block.number, "CNDY::getPriorVotes: not yet determined");
    ...
    function _writeCheckpoint(
        ...
        internal
    {
        uint32 blockNumber = safe32(block.number, "CNDY::_writeCheckpoint: block number exceeds 32 bits");
...
// File: browser/CandyMasterFarmer.sol

pragma solidity 0.6.12;


interface IMigratorToCandySwap {
    // Perform LP token migration from legacy UniswapV2 to CandySwap.
...
// CandyMasterFarmer is the master of Candy. He can make Candy and he is a fair guy.
//
// Note that it's ownable and the owner wields tremendous power. The ownership
// will be transferred to a governance smart contract once Candy is sufficiently
// distributed and the community can show to govern itself.
//
contract CandyMasterFarmer is Ownable, Authorizable {
    using SafeMath for uint256;
    ...
    uint256[] public REWARD_MULTIPLIER =[128, 128, 64, 64, 64, 64, 64, 64, 64, 64, 64, 64, 64, 64, 64, 64, 64, 32, 32, 32, 32, 32, 32, 32, 64, 64, 64, 64, 64, 64, 64, 64, 64, 64, 64, 32, 32, 16, 16, 16, 32, 32, 64, 64, 64, 64, 64, 128, 128, 128, 128, 128, 128, 128, 128, 64, 64, 64, 128, 128, 128, 128, 128, 128, 128, 128, 128, 64, 64, 64, 64, 64, 64, 64, 64, 64, 64, 64, 32, 32, 32, 32, 32, 32, 32, 32, 32, 32, 32, 32, 32, 16, 16, 16, 16, 8, 8, 8, 4, 2, 1, 0];
    ...
    IMigratorToCandySwap public migrator;
    ...
    uint256 private halvingAfterBlock;
    event Deposit(address indexed user, uint256 indexed pid, uint256 amount);
    event Withdraw(address indexed user, uint256 indexed pid, uint256 amount);
    event EmergencyWithdraw(address indexed user, uint256 indexed pid, uint256 amount);
    event SendCandyReward(address indexed user, uint256 indexed pid, uint256 amount);
    ...
    constructor(
        ...
        uint256[] memory _treasuryPercents,
        ...
        PERCENT_FOR_DEV = _treasuryPercents[0];
        PERCENT_FOR_LP= _treasuryPercents[1];
        PERCENT_FOR_COM = _treasuryPercents[2];
        PERCENT_FOR_FOUNDERS = _treasuryPercents[3];
        userDepFee = _depositFees[0];
        devDepFee = _depositFees[1];
    ...
    function add(uint256 _allocPoint, IBEP20 _lpToken, bool _withUpdate) public onlyOwner {
        require(poolId1[address(_lpToken)] == 0, "CandyMasterFarmer::add: lp is already in pool");
        if (_withUpdate) {
            massUpdatePools();
        }
    ...
    function setMigrator(IMigratorToCandySwap _migrator) public onlyOwner {
        migrator = _migrator;
    }
    ...
    function updatePool(uint256 _pid) public {
        ...
        uint256 CandyForDev;
        uint256 CandyForFarmer;
        uint256 CandyForLP;
        uint256 CandyForCom;
        uint256 CandyForFounders;
        (CandyForDev, CandyForFarmer, CandyForLP, CandyForCom, CandyForFounders) = getPoolReward(pool.lastRewardBlock, block.number, pool.allocPoint);
        Candy.mint(address(this), CandyForFarmer);
        pool.accCandyPerShare = pool.accCandyPerShare.add(CandyForFarmer.mul(1e12).div(lpSupply));
        pool.lastRewardBlock = block.number;
        if (CandyForDev > 0) {
            Candy.mint(address(devaddr), CandyForDev);
        }
        if (CandyForLP > 0) {
            Candy.mint(liquidityaddr, CandyForLP);
        }
        if (CandyForCom > 0) {
            Candy.mint(comfundaddr, CandyForCom);
        }
        if (CandyForFounders > 0) {
            Candy.mint(founderaddr, CandyForFounders);
        }
    }
    ...
    function getPoolReward(uint256 _from, uint256 _to, uint256 _allocPoint) public view returns (uint256 forDev, uint256 forFarmer, uint256 forLP, uint256 forCom, uint256 forFounders) {
        ...
        uint256 CandyCanMint = Candy.cap().sub(Candy.totalSupply());

        if (CandyCanMint < amount) {
            forDev = 0;
            forFarmer = CandyCanMint;
            forLP = 0;
            forCom = 0;
            forFounders = 0;
        }
    ...
    function pendingReward(uint256 _pid, address _user) external view returns (uint256) {
        ...
        if (block.number > pool.lastRewardBlock && lpSupply > 0) {
            uint256 CandyForFarmer;
            (, CandyForFarmer, , ,) = getPoolReward(pool.lastRewardBlock, block.number, pool.allocPoint);
            accCandyPerShare = accCandyPerShare.add(CandyForFarmer.mul(1e12).div(lpSupply));
    ...
    function _harvest(uint256 _pid) internal {
        ...
        if (user.amount > 0) {
            ...
            uint256 masterBal = Candy.balanceOf(address(this));
            ...
            if(pending > 0) {
                Candy.transfer(msg.sender, pending);
                user.rewardDebtAtBlock = block.number;
                emit SendCandyReward(msg.sender, _pid, pending);
            }
    ...
    // Deposit LP tokens to CandyMasterFarmer for $CNDY allocation.
    function deposit(uint256 _pid, uint256 _amount) public {
        require(_amount > 0, "CandyMasterFarmer::deposit: amount must be greater than 0");
        ...
        devr.rewardDebt = devr.amount.mul(pool.accCandyPerShare).div(1e12);
  ...
  // Withdraw LP tokens from CandyMasterFarmer.
    function withdraw(uint256 _pid, uint256 _amount) public {
        PoolInfo storage pool = poolInfo[_pid];
        UserInfo storage user = userInfo[_pid][msg.sender];
        UserGlobalInfo storage current = userGlobalInfo[msg.sender];
        require(user.amount >= _amount, "CandyMasterFarmer::withdraw: not good");
